...
mousereport(XEvent *e)
{
	...
	if (e->type == MotionNotify) {
		...
		/* Only buttton 1 through 11 can be encoded */
		if (btn < 1 || btn > 11)
			return;
		if (e->type == ButtonRelease) {
			...
			if (IS_SET(MODE_MOUSEX10))
				return;
			/* Don't send release event for the scroll wheel*/
			if (btn == 4 || btn == 5)
				return;
		}
		code = 0;
	...
	/* Encoded btn into code. If no button is pressed for a motion event in
...
{
	if (dc.collen == 0 || !BETWEEN(x, 0, dc.collen - 1))
		return 1;
